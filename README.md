...
- **录入/编辑**：持卡人姓名、卡号、有效期（MM/YY）、CVV（可选）、发卡行、备注
- **自动识别**
...
- **卡片墙**：钱包式堆叠列表，上下滑动浏览；点击卡片弹出（缩放+淡入动画），再点卡面**翻转**看卡背 CVV
...
- **到期分类**：30 天内到期的卡自动归入「即将到期」（带数量角标），已过期单独分类，卡面显示角标
- **安全**
...
- **设置**：代理（`socks5://127.0.0.1:10808` 或 `http://host:port`，含连通性测试）、生物识别开关、自动锁定时长、剪贴板清除时长、打码开关、防截屏开关、主题（跟随系统/浅色/深色）、修改 PIN

## 隐私说明

- 卡片数据只写入本机加密数据库，应用无任何统计/上报。
- 唯一联网行为是用户主动触发的 BIN 查询和代理连通测试；BIN 查询只发送卡号前 8 位。
...
- 后续可加：加密导出/导入备份、到期本地通知（WorkManager）、卡片拖拽排序、NFC 读卡自动录入。
